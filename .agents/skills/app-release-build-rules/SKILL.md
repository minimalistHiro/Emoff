...
description: emoffプロジェクトで「アプリをリリースして」等の依頼が来たときに、Google Play/App Store向けのリリースビルド手順を実行するために使う。
---
...
emoffのリリース依頼時に、AndroidのAAB作成とiOSビルド準備を順番に実行する。iOSのArchiveはコマンドで作成し、Xcode Organizer に表示されるようアーカイブ場所へコピーして開くところまで行う。
...
2. `ios`ディレクトリで`pod install`を実行する（`cd /Users/kanekohiroki/Desktop/emoff/ios && pod install`）。
3. `flutter build ios`を実行し、App Storeリリース向けのビルドを行う。
4. Xcode Organizer に表示されるよう、既定の Archives 直下へアーカイブを作成して開く。
   - 例:
