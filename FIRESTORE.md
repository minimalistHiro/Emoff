...
| `messages`（collectionGroup） | `originalText` ASC, `createdAt` ASC | originalText自動削除用（1ヶ月経過分の一括取得） |
...
- Cloud Functions `deleteExpiredOriginalTexts`（毎日午前3時JST実行）で `createdAt` が1ヶ月以上前かつ `originalText` が null でないメッセージの `originalText` を `null` に更新
- 実装: `functions/src/index.ts` / リージョン: `asia-northeast1` / デプロイ済み（2026-04-10）
