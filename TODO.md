# TODO リスト

> 作成日: 2026-04-01

---

## ルール

- 各項目には以下を記載する:
  - **優先度**: 最高 / 高 / 中 / 低（長期）のいずれか
  - **内容**: 実装・対応内容の概要
  - **状態**: `[ ] 未着手` / `[ ] 計画済み（日付）` / `[ ] 進行中` / `[x] 完了（日付）` / `[x] 廃止（日付）`
- 完了・廃止になった項目は COMPLETED.md に移動する。
- **IDの再利用は禁止**。TODO.md と COMPLETED.md の両方を確認し、既存の最大番号の次を使用すること。

---

## タスク一覧

<!-- 例: | A-1 | 高 | チャット画面の実装 | [ ] 未着手 | -->

| ID | 優先度 | 内容 | 状態 |
|---|---|---|---|
| A-1 | 最高 | スプラッシュ画面の実装（認証状態判定・遷移分岐） | [ ] 未着手 |
| A-2 | 最高 | ログイン画面の実装（メール+パスワード認証） | [ ] 未着手 |
| A-3 | 最高 | 新規登録画面の実装（メール+パスワード登録） | [ ] 未着手 |
| A-4 | 最高 | パスワードリセット画面の実装 | [ ] 未着手 |
| A-5 | 最高 | Firebase Authentication の初期設定（メール+パスワード有効化） | [ ] 未着手 |
| A-6 | 高 | 新規登録フローにAI処理同意ポップアップの実装（利用規約同意後に表示） | [ ] 未着手 |
| A-7 | 高 | 変換前テキストの自動削除機能の実装（Firestoreに1ヶ月保存後に自動削除） | [ ] 未着手 |
| A-8 | 最高 | Firestore初期設定（asia-northeast1リージョン、コレクション・インデックス設定） | [ ] 未着手 |
| A-9 | 高 | Firestoreセキュリティルールのデプロイ | [ ] 未着手 |
| A-10 | 高 | 非正規化データ同期用Cloud Functionsの実装（プロフィール更新時のfriends/friend_requests同期） | [ ] 未着手 |
| A-11 | 高 | アカウント削除時のデータクリーンアップCloud Functionsの実装 | [ ] 未着手 |
| A-12 | 高 | ブロック機能の実装（blocked_usersサブコレクション、友達削除連動、メッセージ非表示） | [ ] 未着手 |
| A-13 | 高 | 通報機能の実装（reportsコレクション、通報理由選択UI） | [ ] 未着手 |
| A-14 | 高 | Freeプランのメッセージ送信上限制御の実装（1日30通、上限到達時のUI表示） | [ ] 未着手 |
| A-15 | 高 | アプリ内課金（サブスクリプション）の実装（App Store / Google Play対応） | [ ] 未着手 |
| A-16 | 中 | 変換トーン選択機能の実装（ビジネス敬語/カジュアル等、Personal Pro向け） | [ ] 未着手 |
